// 안부확인콜(TTS_CALLS) — POST /health/tts-calls/schedule. 이 엔드포인트는 인증을 요구하지
// 않는다(백엔드 health/router.py 주석: DB 조회 없이 user_id만 받아 호출을 예약/발신하므로
// 개인정보 노출 위험이 없다고 판단). scheduled_at을 "지금"으로 보내면 alerts_service.py의
// schedule_tts_call()이 대상자 tts_call_consent가 켜져 있는 경우 즉시 Azure로 실제 발신한다
// (동의 안 했으면 이력만 남기고 발신은 건너뜀 — 백엔드가 알아서 처리, 프론트는 신경 쓸 필요 없음).
...
// 백엔드 호출 자체엔 토큰이 필요 없지만, 어느 대상자인지(backendWardId)는 알아야 하고 그
// 대상자가 백엔드에 아직 없으면(한 번도 다른 기능을 안 써봤으면) 새로 만들어야 하는데, 그
// 확보 과정(ensureBackendWardId)엔 보호자/본인 세션이 필요하다 — resolveBackendWardAccess를
// 그대로 재사용하는 이유.
