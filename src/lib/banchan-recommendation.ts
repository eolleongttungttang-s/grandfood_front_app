...
//
// 원래 "오늘"(todayDateString()) 하나만 고정으로 계산했는데, records-view.tsx의 "최근 7일
// 달성률" 막대 그래프(2026-08-19 추가)가 과거 여러 날짜에 대해 같은 계산을 반복해야 해서
// 날짜를 매개변수로 뺐다. computeTodayNutritionSnapshot은 그 특수 케이스(dateStr = 오늘)로
// 남겨 기존 호출부(records-view.tsx의 "오늘 영양성분 분석", health-insights.ts)는 그대로 둔다.
...
// "최근 N일 달성률" 막대(records-view.tsx)용 — 그날 배정된 4개 영양소 중 목표치가 있는
// 것만 골라 "실제/목표" 비율을 각각 100%로 캡한 뒤 평균낸다. 4개 영양소를 하나의 숫자로
// 뭉뚱그리는 이유는, 막대 하나에 4개 값을 다 못 넣으니 "그날 전체적으로 얼마나 목표에
// 가까웠는지" 한 눈에 보이는 요약이 더 유용하기 때문이다(개별 영양소는 같은 화면의 "오늘
// 영양성분 분석" 카드에서 볼 수 있다). hasData가 false거나 목표치가 하나도 없으면(건강
// 프로필 미입력 등) null — 막대를 안 그리는 신호로 쓴다.
