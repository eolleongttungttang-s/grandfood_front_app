...
import { isUsingAutoGeneratedPassword } from "@/lib/password-security-notice";
import { dismissedSosStore, sosAckStore } from "@/lib/sos-store";
import { currentMealDelivery, type WardStatus } from "@/lib/ward-registry";
...
const TYPE_STYLE: Record<NotificationType, string> = {
  ...
  보안: "bg-risk-caution text-risk-caution-foreground",
...
export function getElderDeliveryNotification(status: WardStatus): NotificationItem {
  // 끼니 이름과 시각을 currentMealDelivery() 한 번으로 같이 받는다 — 각자 따로
  // new Date()를 불러 계산하면 11시/17시 경계에서 서로 다른 끼니로 갈릴 수 있다
  // (코드 리뷰 지적, 2026-08-27).
  ...
  return {
    ...
    date: formatOccurredAt(new Date().toISOString()),
    type: "배송",
    message: `오늘 ${mealLabel} 배송이 ${eta}에 예정되어 있어요.`,
    read: false,
  };
}
...
// 완식/배송/SOS확인과 같은 프론트 합성 알림 — QR초대로 가입한 어르신 계정은 비밀번호가
// 전화번호 뒷자리로 자동 생성돼 제3자가 추측하기 쉽다(password-security-notice.ts 상단
// 주석 참고, 2026-08-27 보안 점검). 처음엔 홈 화면에 상시 배너로 올렸는데, "화면 정보를
// 최소화해야 한다"는 피드백으로 다른 안내 사항과 같은 방식(알림함)으로 옮겼다. id를
// 날짜 없이 고정해서, 완식/배송과 달리 한 번 알림함을 열어보면(markNotificationsSeen)
// 종 아이콘 배지는 다시 안 켜진다 — 다만 실제로 비밀번호를 바꿀 방법이 아직 없어(위 주석
// 참고) read는 계속 false로 둬서, 알림함 목록 안에서는 매번 "아직 안 끝난 항목"으로
// 보이게 한다.
export function getElderPasswordSecurityNotice(account: Account | null): NotificationItem | null {
  if (!account || !isUsingAutoGeneratedPassword(account)) return null;
  return {
    id: "password-security-notice",
    date: formatOccurredAt(new Date().toISOString()),
    type: "보안",
    message:
      "지금 비밀번호는 전화번호 뒷자리로 자동으로 만들어졌어요. 다른 사람이 알아내기 쉬우니, 자녀나 보호자에게 비밀번호를 바꿔달라고 말씀해 주세요.",
    read: false,
  };
}
