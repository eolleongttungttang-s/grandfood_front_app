...
export function RecordsView({
  ...
  // 기본으로 "오늘"(그리드 맨 끝 칸)을 펼쳐 보여준다 — banchan-recommendation-calendar.tsx의
  // 날짜 상세 기본값(pickDefaultDate)과 같은 이유로, 진입하자마자 가장 궁금할 날짜를 바로
  // 보여준다.
  ...
  // "오늘 영양성분 분석" 위에 최근 며칠 추이를 막대 그래프로 같이 보여준다(2026-08-19
  // 피드백 두 차례: 처음엔 식단(diet) 화면의 AI 반찬 추천 달력에 막대+테두리 박스로 넣었다가
  // "그래프까지는 필요 없고 박스가 별로다"는 의견에 숫자(%)만 남기고 이 화면으로 옮겼는데,
  // 다시 "막대그래프로 보여주는 게 낫겠다"는 의견을 받아 막대를 되살렸다 — 다만 겹박스가
  // 되지 않게 별도 카드 하나로만 감싼다). 오늘을 마지막 칸으로 최근 NUTRITION_TREND_DAYS일의
  // "평균 목표 달성률"(computeAchievementPct — 4개 영양소 중 목표가 있는 것만 각각 100%로
  // 캡한 뒤 평균)을 구한다.
      ...
      <div className="flex flex-col gap-4 px-5">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-5 shadow-sm">
          ...
          {/* 칸이 title 툴팁(마우스 오버)만으로 하루 상태를 알려줘서 터치 기기에선 사실상
              ...
              보여주도록 바꿨다(2026-08-19 피드백). banchan-recommendation-calendar.tsx의
              "날짜 칸 탭 → 아래 상세" 패턴을 그대로 따른다. */}
        ...
        {/* "최근 14일 섭취 기록"(완식/소량/미응답)과 "오늘 영양성분 분석" 사이에 둔다 —
            "이건 오늘 영양분석 위에 있는 게 좋겠고, 막대그래프로 보여주는 게 낫겠다"는
            피드백(2026-08-19)을 반영했다. 페이지의 다른 카드들과 같은 테두리 하나만
            두르고(rounded-2xl border ... 컨벤션), 그 안에 막대까지 바로 넣어서 "박스 안에
            또 박스"가 되지 않게 한다(막대 자체를 감싸는 별도 테두리 없음). */}
        {hasNutritionTrend && (
          <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-5 shadow-sm">
