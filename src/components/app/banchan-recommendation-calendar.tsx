...
// 기본으로 펼쳐 보여줄 날짜 — 오늘이 이번 달 데이터 안에 있으면 오늘, 없으면(예: 이번 달의
// 첫 몇 칸이 지난달 소속 주라 아직 이번 달로 안 들어온 극히 드문 경우) 완료된 첫 날짜,
// 그것도 없으면 첫 칸.
...
export function BanchanRecommendationCalendar({
  ...
  const days = useMemo(() => buildDayCells(monthly), [monthly]);
  const weeks = useMemo(() => {
  ...
  // 조정하기" 패턴) — 지금 선택이 이번 달 안에 여전히 유효하면 그대로 두고, 무효할 때만
  // (월이 바뀌었거나 최초 진입) 기본값을 새로 고른다. 최초 마운트분은 위 useState 지연
  ...
  const selectedWeek = selected ? monthly.weeks.find((w) => w.weekStartDate === selected.weekStartDate) : undefined;
