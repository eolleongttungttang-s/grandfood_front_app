...
// 바꿨다. Ward.age는 생년월일이 아니라 숫자로만 저장되므로(wards.ts), 여기서 고른 생년월일은
// 제출 시점에 calculateAge()로 나이를 계산해서 그 숫자만 저장한다 — signup의 createSelfWard가
// 하는 것과 동일한 변환이다. 그래서 이 화면을 다시 열면 원래 생년월일은 복원되지 않고
// 빈 선택창으로 시작한다(가입 화면도 재방문 시 이전 값이 안 남는 것과 같은 한계).
    ...
    <div className="flex flex-1 flex-col">
      ...
      <main className="flex flex-1 flex-col px-5 py-6">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="profile-edit-address">거주지</Label>
            <Input
